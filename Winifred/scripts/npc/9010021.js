    ...
    if (status == 0) {
        ...
        if (selection == 0) {
            if (cm.getChar().getLevel() >= 200) {
                ...
                cm.sendOk("You are not at least level 200, please come back when you hit level 200.");
                cm.dispose();
            }
        } else if (selection == 1) {
            cm.sendOk("Ok Bye")
        ...
        var reborns = cm.getChar().getReborns();
        if (reborns >= 6) {
            cm.sendOk("You have already been reborn 6 times, you can't rebirth again.");
            cm.dispose();
        } else {
            cm.sendOk("You have now been reborn. That's a total of #r" + reborns + "#k rebirths");
            cm.dispose();
        }
