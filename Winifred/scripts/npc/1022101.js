    ...
    if (mode < 0)
        ...
        if (status == 0 && mode == 1) {
			if(cm.getPlayer().getLevel() < 20) {
				cm.sendOk("Hello, I am the Vote Point exchanger for #Olympus#k!\r\n\r\nI am sorry, but I can only exchange Vote Points for players #blevel 20 or over#k.");
				cm.dispose();
				return;
			}
			var outStr = "Hello, I am the Vote Point exchanger for #MaryMS#k!\r\n";
			outStr += "You currently have #r" + vp + "#k Vote Points.#b\r\n\r\n";
			outStr += "#L1#I would like to exchange 1 #t" + vp + "# for "+ nxAmount + " NX Cash#l\r\n";
			outStr += "#L2#I would like to exchange 1 #t" + vp + "# for " + chairAmount + " Random Chair" + (chairAmount > 1 ? "s" : "") + "#l\r\n";
			outStr += "#L3#I would like to exchange 1 #t" + vp + "# for " + weaponAmount + " Maple Weapons#l\r\n";
			outStr += "#L4#I would like to exchange 1 #t" + vp + "# for " + buffAmount + " #t" + buff1ID + "#s and " + buffAmount + " #t" + buff2ID + "#s#l\r\n";
			...
			if(selection > 0) {
				if(!cm.haveItem(itemToUse) && vp == 0) {
					cm.sendOk("I'm sorry, but you don't have any Vote Points.");
					cm.dispose();
					return;
				}
			}
			...
			if(selection == 1) {
				...
				cm.sendYesNo("Would you like to exchange 1 #t" + vp + "# for " + nxAmount + " NX Cash?");
			} else if(selection == 2) {
				// Exchange 1 Leaf for Chair
				cm.sendYesNo("Would you like to exchange 1 #t" + vp + "# for " + chairAmount + " Random Chair" + (chairAmount > 1 ? "s" : "") + "?");
			} else if(selection == 3) {
				// Exchange 1 Leaf for Maple Weapons
				cm.sendYesNo("Would you like to exchange 1 #t" + vp + "# for " + weaponAmount + " Random Maple Weapons?");
			} else if(selection == 4) {
				// Exchange 1 Leaf for Apples/Cheese
				cm.sendYesNo("Would you like to exchange 1 #t" + vp + "# for " + buffAmount + " #t" + buff1ID + "# and #t" + buff2ID + "#?");
			} else if(selection == 5) {
				// Echange 1 Leaf for Merchant
				cm.sendYesNo("Would you like to exchange 1 #t" + vp + "# for a " + hiredMerchantLength + " Day Hired Merchant?");
			} else {
				cm.dispose();
			}
		} else if(status == 2) {
			...
			if(choice == 0) {
				...
				if(!cm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).isFull(weaponAmount)) {
					...
					// cm.dispose();
					cm.sendOk("VP weapoons are disabled at this moment in time.");
				} else {
					cm.sendOk("Please make sure you have enough space to hold the items!");
				}
			} else if(choice == 4) {
