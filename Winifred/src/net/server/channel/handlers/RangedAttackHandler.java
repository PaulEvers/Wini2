    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (attack.skill == Buccaneer.ENERGY_ORB || attack.skill == ThunderBreaker.SPARK || attack.skill == Shadower.TAUNT || attack.skill == NightLord.TAUNT) {
            ...
            if (attack.skill != 0) {
                ...
                // Super Trans fix
                if (chr.hasActiveBuff(5121003)) {
                    effect.setCooldown(5000);
                    c.announce(MaplePacketCreator.skillCooldown(attack.skill, effect.getCooldown()));
                    chr.addCooldown(effect.getSourceId(), currentServerTime(), effect.getCooldown() * 10);
                }
                else if (effect.getCooldown() > 0) {
                    c.announce(MaplePacketCreator.skillCooldown(attack.skill, effect.getCooldown()));
                }
